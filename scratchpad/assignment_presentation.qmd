...
That previous slide was confusing 😵‍💫
...
:::{.incremental}
* Deconstructing and explaining tools in simpler terms encourages adoption of the tool.
* There is a gap on the material available to explain how XAI tools work internally in simple terms
* Simpler explanations of complex processes makes the methods more accessible to a wider user base.
* Let's fix that! 🧑🏻‍🔧 👨🏻‍🔧
:::
...
## The brute force approach {.smaller}

*Simply put,* we try every single possible bounding box since there is a finite amount of bounding boxes.

<p style = "text-align:center"><img src = "imgs/1dim_prec_cov.png" style="width:60%" /></p>

## Results for the maximum precision

<p style = "text-align:center"><img src="imgs/1dim_brute_results_2.png" style="width:80%"/></p>

## Results for the optimal bound

<p style = "text-align:center"><img src="imgs/1dim_brute_results_optim_2.png" style = "width:80%"/></p>

## Let's consider a different point

<p style = "text-align:center"><img src="imgs/1dim_alt_here_plot.png" style = "width:80%"/></p>

## Results for two criterias {.smaller}

Left side is the previous point, right side is the alternative point

<p style = "text-align:center"><img src="imgs/1dim_alt_results.png" style = "width:60%"/></p>
...
Until we run out of dimensions... 🤷🏻‍♂️
...
## Final Results {.smaller}

<p style="text-align:center"><img src="assignment_state_plot_1.gif" style="width:60%"/></p>

The agent is maximizing coverage but in the process is capturing the unnecessary points as well thereby reducing the precision. However the reward will be high since more weight is put on the coverage.
...
4. Providing the intuition of anchors in the different scenarios.

## Next steps

1. Provide explanations in three dimensions and higher
2. Implement a pure exploration method for the multi arm bandit problem
3. Explore other alternatives for generating the ideal boundary region in the model space.
