...
```{r}
w <- readRDS(here::here("scratchpad/test_tour_data.rds"))[["model_data"]] |>
  filter(new_cause %in% c("lightning", "accidental_human")) |>
  mutate(new_cause = factor(new_cause))
set.seed(69420)
train_indices <- sample(nrow(w), round(nrow(w) * 0.9))
train_df <- w[train_indices, ] |> mutate(id = row_number())
test_df <- w[-train_indices, ] |> mutate(id = row_number())
```

```{r}
interest_cols <- c("air_tmin", "air_tmax", "radiation", "rainfall")
animate_xy(
  test_df |> select(all_of(interest_cols)) |> tourr::rescale(),
  axes = "bottomleft",
  col = train_df$new_cause
)
```
...
rfmodel <- randomForest(
  new_cause ~ air_tmin + air_tmax + radiation + rainfall,
  data = train_df
)
```

```{r}
prob_matrix <- predict(rfmodel, newdata = test_df, type = "prob") |> 
  as.data.frame() |>
  mutate(id = row_number())
local_instance <- prob_matrix[prob_matrix$lightning < 0.95 & prob_matrix$lightning > 0.85 , "id"]
local_instance <- local_instance[2]
preds <- predict(rfmodel, newdata = test_df)
```

```{r}
light_col <- "#FFB200"
nlight_col <- "#5A31C1"
local_inst_colors <- ifelse(
  test_df$new_cause == "lightning", 
  light_col, nlight_col
)
local_inst_colors[local_instance] <- "#0FA6B3"
...
envir <- map(interest_cols, function(cname) {
  vals <- test_df[-local_instance,][[cname]] |> sort()
  cutpoints <- map2_dbl(vals[-length(vals)], vals[-1], function(x, x_1) {
    return(mean(c(x, x_1)))
  })
  v_l <- sort(
    cutpoints[cutpoints < test_df[local_instance,][[cname]]],
    decreasing = TRUE
  )
  if(length(v_l) == 0) {
    v_l <- c(test_df[local_instance,][[cname]])
  }
  v_u <- cutpoints[cutpoints > test_df[local_instance,][[cname]]]
  if(length(v_u) == 0) {
    v_u <- c(test_df[local_instance,][[cname]])
  }
  list(v_l, v_u)
}) |> list_flatten() |>
...
pertub_func <- function(n) {
  out <- mulgar::rmvn(n = n, 
               ...
               mn = test_df[local_instance, interest_cols] |> 
                 unlist(),
               vc = cov(test_df[,interest_cols])
  ) |>
    as.data.frame()
...
set.seed(123)
PRJ <- basis_random(ncol(test_df |> select(all_of(interest_cols))), d = 2)
...
get_reward <- function(new_anchor, round, class_ind = 2) {
  ...
  print(glue::glue("==== {cover} | {prec} | {as.numeric(satisfies(new_anchor, test_df[local_instance, ])) * 
      mean(c(cover / round, prec), na.rm = TRUE) }====="))
  return(list(
    reward = as.numeric(satisfies(new_anchor, test_df[local_instance, ])) * 
      mean(c(cover / round, prec), na.rm = TRUE),
      # prec,
...
for(game in seq_len(n_games)) {
  ...
  sample_data <- rbind(
    gcube$points,
    test_df |> select(all_of(interest_cols))
  ) |> tourr::rescale()
  
  local_inst_colors <- ifelse(preds == "lightning", light_col, nlight_col)
  ...
  render_gif(
    ...
    gif_file= here::here("scratchpad/bushfire_state_plot_dump/",glue::glue("{game}.gif")),
    frames=500,
    width=600,
    height=600,
    loop=TRUE
...
render_gif(
  ...
  gif_file = here::here("scratchpad/bushfire_state_plot_dump/inside_box_model.gif"),
  frames = 500,
  width=600,
  height=600,
  loop=TRUE
...
render_gif(
  ...
  display_scatmat(),
  gif_file=here::here("scratchpad/bushfire_state_plot_dump/box_scatmat.gif"),
  frames=500,
  width=600,
  height=600,
  loop=TRUE
