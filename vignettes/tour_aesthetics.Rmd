...
set.seed(123)
n_points <- 200
dataset <- MASS::mvrnorm(n = n_points, mu = c(0, 0, 0), Sigma = diag(1, 3)) |>
  as.data.frame() |>
  setNames(c("x1", "x2", "x3")) |>
  tibble::tibble()
```
...
Now that we have the ingredients set up. The first step is to create a bounding box instance. 
...
![](tour_aes_1.gif){alt-text="An animation containing a tour of a high dimensional space with a box representing the anchor"}
